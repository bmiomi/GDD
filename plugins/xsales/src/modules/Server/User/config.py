...
class ConfigFtp(Config):
     ...
     def pathdownload(self):
          ...
          path=self.config.get('FTP').get('Repositorio').get('credenciales').get(self.user).get('path',None)
          if  path:
     ...
     @property
     def excluide(self):
          path=self.config.get('FTP').get('Repositorio').get('credenciales').get(self.user).get('excluide')
          if  path:
               return path
     ...
     def CredencialesFtp(self) -> tuple:
          ...
          if 'DESC' in self.__operacion :
               credenciales=self.config.get('FTP').get('Repositorio').get('credenciales').get(self.__user)
          if 'Maestros' in self.__operacion:
               credenciales=self.config.get('FTP').get('Maestros').get(self.__user)
...
class ExcelFile:
     ...
     def excelfile(cls):
          ...
          c=datetime.fromtimestamp(path.getmtime(cls._nombrearchivo)).strftime('%d/%m/%Y')
