...
from core.config import PreferencesManager, QueriesManager
from core.wizard import PreferenceWizard, QueryWizard
...
class ServerModule(XSalesModule):
    ...
    def setup(self, context) -> bool:
        ...
        # Primera ejecución: ejecutar wizard de preferencias
    ...
    def run(self, context) -> Any:
        ...
        # Mostrar menú principal (pasar preferencias)
        self.dato = menu_principal(
            context.questionary,
            self._config,
            self.preferences
        )
        ...
        # Manejar acciones
        if self.dato.action == 'configure':
            return self._handle_configure(context)
        ...
        # Usar preferencias para generar archivo
        if self.dato.reporte or (self.preferences and self.preferences.generate_excel):
            self.generararchivo(
                True,
                self.dato.Opcion,
                self.dato.Turno,
    ...
    def _handle_configure(self, context) -> Any:
        ...
        context.console.print("[bold cyan]Configurar consultas[/bold cyan]")
        
        accion = context.questionary.select(
            "¿Qué deseas hacer?",
            choices=['Crear nueva consulta', 'Ver consultas guardadas', 'Eliminar consulta']
        ).ask()
        ...
        if accion == 'Crear nueva consulta':
            ...
            if query:
                self.queries_manager.save(self.metadata.name, query)
                context.console.print(f"[green]✓ Consulta '{query.name}' guardada[/green]")
        
        elif accion == 'Ver consultas guardadas':
            queries = self.queries_manager.load_all(self.metadata.name)
            if queries:
            ...
            if queries:
                ...
                self.queries_manager.delete(self.metadata.name, query_name)
                context.console.print(f"[green]✓ Consulta '{query_name}' eliminada[/green]")
        
        return {"status": "configure", "data": None}
    ...
    def consulta_Basedatos(self, nombredz, xsales, console):
        ...
        try:
            ...
            try:
                ...
                # 🔍 MOSTRAR SQL GENERADO (si está habilitado en preferencias)
                if self.preferences and self.preferences.show_sql:
            ...
            if dataset:
                ...
                # Mostrar datos usando presentador según preferencias
                self._present_result(clave_real, dataset, execution_time, console)
            else:
    ...
    def _present_result(self, query_name: str, dataset: list, execution_time: float, console):
        """Presenta resultados usando el formato configurado en preferencias."""
        if not dataset:
            return
        ...
        # Seleccionar presentador según preferencias
        if self.preferences:
            format_type = self.preferences.output_format
        else:
            format_type = 'table'  # Default
        
        if format_type == 'tabla' or format_type == 'table':
            presenter = TablePresenter(console)
        elif format_type == 'lista' or format_type == 'list':
            presenter = ListPresenter(console)
        elif format_type == 'resumen' or format_type == 'summary':
            presenter = SummaryPresenter(console)
        else:
            presenter = TablePresenter(console)  # Fallback
        
        # Mostrar resultado
        presenter.present(result_data)
    ...
    def generararchivo(self, respuesta, nombre: str, turno: str, console):
        ...
        if respuesta:
            ...
            try:
                ...
                # Usar directorio de preferencias si existe
                excel_dir = self.preferences.excel_directory if self.preferences else './REPORTES'
                archivo = self._config.path.join(excel_dir, f'{nombre}')
